...
// ######## load the elements
void load(char *filename, int N, char *filenamedens)
{
...
cout<<"reading "<<N<<" lines from  "<<filename<<"\n" ;
 ...
 ifstream fin_my(filenamedens);
 //ifstream fin_my("input/dens_test.dat");
 if(!fin){ cout << "cannot read file " << filename << endl ; exit(1) ; }
 if(!fin_my){ cout << "cannot read test file " << filename << endl ; exit(1) ; }
 ...
 for(int n=0; n<Nelem; n++){
   ...
   instream.clear() ; // does not work with gcc 4.1 otherwise
    instream>>surf[n].tau>>surf[n].x>>surf[n].y>>surf[n].eta
      ...
      double uu2  = surf[n].u[1]*surf[n].u[1]+surf[n].u[2]*surf[n].u[2]+surf[n].u[3]*surf[n].u[3];
	if (uu2 > surf[n].u[0]*surf[n].u[0] ) 
	{
	surf[n].u[1] = surf[n].u[1]*0.99;
	surf[n].u[2] = surf[n].u[2]*0.99;
	surf[n].u[3] = surf[n].u[3]*0.99;
	}

      for(int i=0; i<10; i++) instream>>surf[n].pi[i] ;
      instream>>surf[n].Pi ;
      if(surf[n].muq>0.12){ surf[n].muq=0.12 ; // limit charge ch.pot.
	ncut++ ;
      }
fin_my>>surf[n].rhoe>>surf[n].rhob>>surf[n].rhoq>>surf[n].rhos>>surf[n].press;
   if(instream.fail()){ cout<<"reading failed at line "<<n<<"; exiting\n" ; exit(1) ; }
   
   // print approx. freeze-out profile!
   //if ((surf[n].x-surf[n].y)/fabs(surf[n].x)<0.000001 && surf[n].eta==0 && surf[n].x>0 && surf[n].y>0) cout << sqrt(surf[n].x*surf[n].x+surf[n].y*surf[n].y) << " " << surf[n].tau << endl;
...
int generate()
{
 ...
 int nmaxiter = 0 ;
 double netBaryon=0.0, netStrangeness=0.0, netElectric=0.0 ;
 for(int iel=0; iel<Nelem; iel++){ // loop over all elements
  // ---> thermal densities, for each surface element
   totalDensity = 0.0 ;
   double dNetBaryon = 0.0, dNetStrangeness = 0.0, dNetElectric = 0.0 ;
   //surf[iel].T = 0.1647 ;
   for(int ip=0; ip<NPART; ip++){
    ...
    const double stat = int(2.*J) & 1 ? -1. : 1. ;                                             //surf[iel].mus
    ...
    for(int i=1; i<11; i++){
	if ((surf[iel].T>0.01) && (surf[iel].u[0]==surf[iel].u[0]) ){
    ...
    if(density<0.){
      cout << "gen::Negative density\n" ;
    }
    }
    }

if ((surf[iel].T<0.01) && (surf[iel].u[0]==surf[iel].u[0]) ){
        if  ( particle->GetPDG() == 2212 ){  //proton
                dNetElectric += 0.4 * surf[iel].rhob;
                density = 0.4*surf[iel].rhob;
                density = 0;
                }
        if  ( particle->GetPDG() == 2112 ){  //neutron
                dNetBaryon += 0.6 * surf[iel].rhob;
                density = 0.6*surf[iel].rhob;
                //density = 0;
                }
        }


    dNetBaryon += particle->GetBaryonNumber()*density ;
    dNetStrangeness += particle->GetStrangeness()*density ;
    dNetElectric += particle->GetElectricCharge()*density ;
 ...
 // ---< end thermal densities calc
  ...
  netBaryon += dNetBaryon*dvEff ;
  netStrangeness += dNetStrangeness*dvEff ;
  netElectric += dNetElectric*dvEff ;
  ...
  while(cumulantDensity[isort]<xsort) isort++ ;
   ...
   double Temp = surf[iel].T;
    double muf = part->GetBaryonNumber()*surf[iel].mub + part->GetStrangeness()*surf[iel].mus +
               part->GetElectricCharge()*surf[iel].muq ;

   if (surf[iel].T < 0.01){
                if  ( part->GetPDG() == 2212 )  {  //proton
                        Temp = 2.0*3.1415/(mass*gevtofm*gevtofm)*pow(surf[iel].rhob/2.0,2.0/3.0);
                        muf = Temp*log(0.4);
                        }
                if  (part->GetPDG() == 2112) {  //neutron
                        Temp = 2.0*3.1415/(mass*gevtofm*gevtofm)*pow(surf[iel].rhob/2.0,2.0/3.0);
                        muf = Temp*log(0.6);
                        }
   ...
   //do{ // fast momentum generation loop
	// https://arxiv.org/abs/nucl-th/0608057v4 for nonspace-like surface
	double p = 0;
	double phi = 0;
	double sinth = 0;
	bool Wtest = false;
	...
	//	double lambda = (surf[iel].dsigma[1]*mom.Px()+ surf[iel].dsigma[2]*mom.Py() + surf[iel].dsigma[3]*mom.Pz())/(surf[iel].dsigma[0]*mom.E());
	//	if (abs(lambda) < 1){
		//cout << lambda << endl;
	//cout << surf[iel].dsigma[0] << "  "<< surf[iel].dsigma[1] << "  " <<   surf[iel].dsigma[2] <<"  " <<  surf[iel].dsigma[3] << endl; 
	//cout << surf[iel].dsigma[0]*surf[iel].dsigma[0] -(surf[iel].dsigma[1]*surf[iel].dsigma[1] + surf[iel].dsigma[2]*surf[iel].dsigma[2] + surf[iel].dsigma[3]*surf[iel].dsigma[3]) << endl; 
	//		double Wrand = rnd->Rndm()*(1+sqrt(surf[iel].dsigma[1]*surf[iel].dsigma[1] + surf[iel].dsigma[2]*surf[iel].dsigma[2] + surf[iel].dsigma[3]*surf[iel].dsigma[3])/(surf[iel].dsigma[0]*surf[iel].dsigma[0]));
			// Wrand should be less that Wres
	//		double Wres = 1 - lambda;
	//		if (Wrand < Wres) {
	//			Wtest = true;
	//		}	
	//	}
	//	else Wtest = true;
	//	}
   double alpha = abs(surf[iel].dsigma[0])+sqrt(surf[iel].dsigma[1]*surf[iel].dsigma[1]+surf[iel].dsigma[2]*surf[iel].dsigma[2]+surf[iel].dsigma[3]*surf[iel].dsigma[3]);
   // alpha is max(W)
   W = ( surf[iel].dsigma[0]*mom.E() + surf[iel].dsigma[1]*mom.Px() +
        surf[iel].dsigma[2]*mom.Py() + surf[iel].dsigma[3]*mom.Pz() ) / mom.E() ;
   double lambda = -(surf[iel].dsigma[1]*mom.Px() +
        surf[iel].dsigma[2]*mom.Py() + surf[iel].dsigma[3]*mom.Pz() ) / mom.E() ;
   if (abs(lambda)>1){
	W = surf[iel].dsigma[0];
	alpha = surf[iel].dsigma[0];
	}
   double WviscFactor = 1.0 ;
   if(params::shear){
    const double feq = C_Feq/( exp((sqrt(p*p+mass*mass)-muf)/Temp) - stat ) ;
    double pipp = 0 ;
    double momArray [4] = {mom[3],mom[0],mom[1],mom[2]} ;
    for(int i=0; i<4; i++)
    for(int j=0; j<4; j++)
     pipp += momArray[i]*momArray[j]*gmumu[i]*gmumu[j]*surf[iel].pi[index44(i,j)] ;
    WviscFactor = (1.0 + (1.0+stat*feq)*pipp/(2.*Temp*Temp*(params::ecrit*1.15))) ;
   ...
   niter++ ;
	//cout << niter<<endl;
	//cout << 1 - W/surf[iel].dsigma[0] << endl;
 ...
 delete fthermal ;
 cout << " net baryon  "<<netBaryon<<"  strangeness  "<<netStrangeness<<"  electric  "<<netElectric<<endl ;
// exit(1) ;
 return npart[0] ;
}
