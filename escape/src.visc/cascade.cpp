...
for(int iiter=0; iiter<3; iiter++){
 ...
 if(p->def->GetWidth()>0. && !isStable(p->def->GetPDG())){
  ...
  double trnd = (random3->Exp(1./(5.*p->def->GetWidth())))/sqrt(1.-(p->px*p->px+p->py*p->py+p->pz*p->pz)/(p->e*p->e)); 
  p->x = p->x + p->px/p->e*trnd;
  p->y = p->y + p->py/p->e*trnd;
  p->z = p->z + p->pz/p->e*trnd;
  p->t = trnd + p->t;
