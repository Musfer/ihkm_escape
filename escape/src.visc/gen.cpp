...
if ((surf[iel].T<0.01) && (surf[iel].u[0]==surf[iel].u[0]) ){
 ...
 // ---< end thermal densities calc
  ...
  while(cumulantDensity[isort]<xsort) isort++ ;
   ...
   double alpha = (surf[iel].dsigma[1]*mom.Px() +
        surf[iel].dsigma[2]*mom.Py() + surf[iel].dsigma[3]*mom.Pz())/(surf[iel].dsigma[0]*mom.E());
   if (abs(alpha) < 1) {
		W = ( surf[iel].dsigma[0]*mom.E() + surf[iel].dsigma[1]*mom.Px() +
        		surf[iel].dsigma[2]*mom.Py() + surf[iel].dsigma[3]*mom.Pz() ) / mom.E() ;
		//W *= (1-alpha);
		dsigmaMax = 3*(abs(surf[iel].dsigma[0]) + sqrt(surf[iel].dsigma[1]*surf[iel].dsigma[1]+surf[iel].dsigma[2]*surf[iel].dsigma[2]+
				+surf[iel].dsigma[3]*surf[iel].dsigma[1]));
		}
   else {
		W = surf[iel].dsigma[0];
		dsigmaMax = 3*abs(surf[iel].dsigma[0]);
	}	
   ...
   if(params::shear){
    ...
    //if ((WviscFactor > 2.5) and (niter == 0)) cout<<"p ="<< p<< ";  Wvisc = " << WviscFactor << endl;
    if(WviscFactor<0.1) WviscFactor = 0.1 ; // test, jul17; before: 0.5
    //if (niter < 1) cout << "particle" << endl;
