...
int generate()
{
 ...
 int nmaxiter = 0 ;
 double NCFP = 0.0 ; // CFP test for pions
 TH1D *hRapidityDistPos = new TH1D("rapDistP","rapDistP",40,-2.,2.) ;
 TH1D *hRapidityDistNeg = new TH1D("rapDistN","rapDistN",40,-2.,2.) ;
 ...
 for(int iel=0; iel<Nelem; iel++){ // loop over all elements
  // ---> thermal densities, for each surface element
   totalDensity = 0.0 ;
   double dN_MCpos = 0.0, dN_MCneg = 0.0 ;
   double dNetBaryon = 0.0, dNetStrangeness = 0.0, dNetElectric = 0.0 ;
   //surf[iel].T = 0.1647 ;
   for(int ip=327; ip<328; ip++){ // 327 for pi-
    double density = 0. ;
   ...
   // ---- here we calculate dN averaged for each surface element
   // ---- for the analysis of negative contributions to CooperFrye 
   const double _dN = (surf[iel].dsigma[0]*E+surf[iel].dsigma[1]*px
         +surf[iel].dsigma[2]*py+surf[iel].dsigma[3]*pz)*p*p/E
       *c1*(2.*J+1.)/(exp((E-muf)/surf[iel].T)-stat) ;
    // choose only positive/negative contribution (see IF condition below)
   if(_dN>0.) dN_MCpos += _dN*4.0*TMath::Pi()*10.0/1000.0 ;
   if(_dN<0.) dN_MCneg += _dN*4.0*TMath::Pi()*10.0/1000.0 ;
 ...
 // ---< end thermal densities calc
  ...
  NCFP += dN_MCpos ; //dvEff*totalDensity ;
  const double etaShift = params::deta*(-0.5+rnd->Rndm()) ;
  hRapidityDistPos->Fill(surf[iel].eta+etaShift,dN_MCpos) ;
  hRapidityDistNeg->Fill(surf[iel].eta+etaShift,dN_MCneg) ;
 ...
 cout << "N_cooperFrye = " << NCFP << endl ;
 TCanvas *cpos = new TCanvas("cpos","cpos") ;
 hRapidityDistPos->Draw() ;
 TCanvas *cratio = new TCanvas("cratio","cratio") ;
 hRapidityDistNeg->Divide(hRapidityDistNeg,hRapidityDistPos) ;
 hRapidityDistNeg->Draw() ;
 theApp->Run() ;
 exit(1) ;
 delete fthermal ;
